...
const defaultProfile = new URL('@/assets/images/default-profile.png', import.meta.url).href
</script>
  ...
  <div class="card flex items-center justify-between rounded-3xl p-6 bg-white">
    <div class="flex items-center gap-3 w-[260px]">
      <div class="flex size-16 shrink-0 rounded-full overflow-hidden bg-desa-foreshadow">
        <img :src="item.profile_picture || defaultProfile" class="w-full h-full object-cover" alt="photo">
      </div>
      <div class="flex flex-col gap-[6px]">
        <p class="font-semibold text-lg leading-[22.5px] w-[184px] truncate">{{ item.user?.name || 'Tidak ada nama' }}</p>
    ...
    <RouterLink :to="{ name: 'manage-head-of-family', params: { id: item.id } }"
