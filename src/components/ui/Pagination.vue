...
<template>
    <nav id="Pagination">
        <ul class="flex items-center gap-3">
            <li class="group">
                <a @click="serverOptions.page = meta.current_page - 1"
            ...
            <li v-for="page in meta.last_page" :key="page" class="group"
                :class="{ 'active' : page === meta.current_page }">
                <a @click="serverOptions.page = page"
                ...
                <span
                    class="text-desadark-green font-semibold group-[.active]:text-desa-foreshadow group-hover:text-desa-foreshadow transition-setup">
                    {{ page }}
                </span>            
            </a>
        </li>
        <li class="group">
            <a @click="serverOptions.page = meta.current_page + 1"
               :class="{ 'cursor-not-allowed': meta.current_page === meta.last_page }"
               class="flex size-11 shrink-0 items-center justify-center rounded-full bg-desa-foreshadow group-hover:bg-desa-dark-green transition-setup">
               <img src="@/assets/images/icons/arrow-left-dark-green.svg"
